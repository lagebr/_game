...
/**
 * PanicHostile is a hostile which moves slowly and randomly around the map. If
 * the hostile is within a certain range of the target however, it will panic
 * and move fast in the opposite direction.
 * 
 * @author Isaac Arvestad
 * @version 2016-05-11
 */
public class PanicHostile extends Hostile {
	...
	/**
	 * Constructs a PanicHostile at a certain position with a certain size,
	 * texture and target.
	 * 
	 * @param x
	 *            The x-position.
	 ...
	 * @param target
	 *            The target of the hostile.
	 */
	public PanicHostile(float x, float y, float width, float height, Texture texture, World world, Entity target) {
		super(texture);
		...
		body = PhysicsHelper.createDynamicBody(this, ScreenGameRenderer.pixelToMeter(x),
				ScreenGameRenderer.pixelToMeter(y), ScreenGameRenderer.pixelToMeter(width),
	...
	public void update(float deltaTime) {
		...
		if (distance < panicDistance) {
			...
			float x = (random.nextInt(2000) - 1000) / 5000f;
			float y = (random.nextInt(2000) - 1000) / 5000f;
			body.applyLinearImpulse(new Vector2(x, y), body.getWorldCenter(), true);
		}
	}
